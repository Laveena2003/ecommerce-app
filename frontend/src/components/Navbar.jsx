...
const Navbar = () => {
  ...
  return (
    <div className='flex item-center justify-between py-5 font-medium'>
        ...
        <ul className='hidden sm:flex gap-5 text-x3 items-center text-gray-700'>

            <NavLink  to='/' className='flex flex-col  items-center gap-1'>
                <p>HOME</p>
                <hr className='w-2/4 border-none h-[3px]  black bg-gray-700 hidden '  />
            </NavLink>
        ...
        <div className={`absolute top-0 right-0 overflow-hidden bg-white transition-all ${visible ? 'w-full' : 'w-0'}` }>
          <div className='flex flex-col text-gray-600'> 
            <div onClick={()=>setVisible(false)} className=' flex items-center gap-4 p-3 cursor-pointer'>
              <img  className='h-9 rotate-180' src={assets.dropdown_icon} alt=''/>
              <p>Back</p>
            </div>
